...
def infer_spec(df: pd.DataFrame, dataset_prefix: str, target: str) -> DatasetSpec:
    ...
    cat_cols = [col for col in model_columns if col != target and col not in numeric_cols]
    return DatasetSpec(
...
def validate_tabsyn_like_info(csv_path: Path, info: dict) -> None:
    ...
    if overlap:
        raise ValueError(f"Columns declared both numerical and categorical: {sorted(overlap)}")
...
def add_constants_and_save(input_path: Path, output_path: Path, spec: DatasetSpec) -> None:
